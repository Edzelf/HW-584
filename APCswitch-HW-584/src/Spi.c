...
void spi_init(void)
{
  ...
  GPIOC->ODR |= 0x02 ;        // 0b00000010 SI=0, SCK=0, -CS=1
  GPIOE->ODR &= (~0x20) ;     // 0b00100000 -RESET=0

  delay_ms_wd ( 250 ) ;       // wait 250ms
  
  // Release ENC28J60 from reset
  GPIOE->ODR |= (uint8_t)0x20; // 0b00100000 -RESET=1

  // wait 50ms
  delay_ms_wd ( 50 ) ;         // Wait 50ms
...
void SpiWriteByte(uint8_t nByte)
{
  ...
  while(bitnum != 0) {
    if (nByte & bitnum) GPIOC->ODR |= 0x08 ;     // If bit is 1 then 
                                                 // SPI SO (ENC28J60 SI) high
    else GPIOC->ODR &= (~0x08) ;                 // else SPI SO low
    nop();
    GPIOC->ODR |= 0x04 ;                         // SCK high
    nop() ;
    GPIOC->ODR &= (~0x04) ;                      // SCK low

    bitnum = (uint8_t)(bitnum >> 1);             // Shift bitnum right one place
                                                 // Once bitnum has shifted to equal 0
						 // we know all 8 bits have been sent
  }
  GPIOC->ODR &= (~0x08) ;                        // SPI SO low on exit
}
...
void SpiWriteChunk(const uint8_t* pChunk, uint16_t nBytes)
{
  ...
  while (nBytes--) {
    ...
    while(bitnum != 0) {
      if (OutByte & bitnum) GPIOC->ODR |= 0x08 ;     // If bit is 1 then
                                                     // SPI SO (ENC28J60 SI) high
      else GPIOC->ODR &= (~0x08) ;                   // else SPI SO low
...
uint8_t SpiReadByte(void)
{
  ...
  while(bitnum != 0) {
    ...
    if (GPIOC->IDR & (uint8_t)0x10) InByte |= bitnum; // SPI incoming bit = 1
