...
*/
// Modified by Ed Smallenburg for PlatformIO, feb-2021
#include "Enc28j60.h"
#include <stm8s.h>
#include "gpio.h"

#define FULLDUPLEX                   // Define HALFDUPLEX if needed
...
// MAC address ordered as defined in UIP modules ([0] is MSB, [5] is LSB)
...
void Enc28j60Init(void)
{
  ...
  // does not expire until 7500 OSC1 clock cycles (300 �s) pass after 
  ...
  Enc28j60WriteReg(BANK3_MAADR5, uip_ethaddr[0]);  // MAC MSB
  Enc28j60WriteReg(BANK3_MAADR4, uip_ethaddr[1]);
  Enc28j60WriteReg(BANK3_MAADR3, uip_ethaddr[2]);
  Enc28j60WriteReg(BANK3_MAADR2, uip_ethaddr[3]);
  Enc28j60WriteReg(BANK3_MAADR1, uip_ethaddr[4]);
  Enc28j60WriteReg(BANK3_MAADR0, uip_ethaddr[5]);  // MAC LSB
...
int16_t Enc28j60Receive ( uint8_t* pBuffer )
{
  int16_t  nBytes;
...
void Enc28j60Send ( uint8_t* pBuffer, uint16_t nBytes )
{
  ...
  // 16 attempts to work around collisions
  {
    ...
    if ( Enc28j60ReadReg ( BANKX_EIR ) & ( 1 << BANKX_EIR_TXERIF ) )
    {
      ...
      // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
      // Set TXRST
      Enc28j60SetMaskReg ( BANKX_ECON1, (uint8_t)( 1 << BANKX_ECON1_TXRST ) ) ;
      // Clear TXRST
      Enc28j60ClearMaskReg ( BANKX_ECON1, (uint8_t)( 1 << BANKX_ECON1_TXRST ) ) ;
      // Clear TXABRT
      Enc28j60ClearMaskReg ( BANKX_EIR, (uint8_t)( 1 << BANKX_ESTAT_TXABRT ) ) ;
      // Clear TXERIF
      Enc28j60ClearMaskReg ( BANKX_EIR, (uint8_t) ( 1 << BANKX_EIR_TXERIF ) ) ;
      // Clear TXIF
      Enc28j60ClearMaskReg ( BANKX_EIR, (uint8_t)( 1 << BANKX_EIR_TXIF ) ) ;
    }
  ...
  //        if (tsv_byte[3] & 0x20) late_collision = 1;
        ...
        if ( Enc28j60ReadReg ( BANKX_EIR ) & ( 1<< BANKX_ESTAT_LATECOL ) )
        {
          late_collision = 1 ;
        }
        else
        {
          break ; // If no Late Collision error leave the for loop
        }
        ...
        if ( txerif_temp && late_collision )
        {
          ...
          Enc28j60ClearMaskReg ( BANKX_EIR, (uint8_t)( 1 << BANKX_EIR_TXIF ) ) ;
          // Start transmission
          Enc28j60SetMaskReg(BANKX_ECON1, (uint8_t)( 1 << BANKX_ECON1_TXRTS ) ) ;
          // Wait for transmission complete ... or collision error
          // while(TXIF == 0 and TXERIF == 0) NOP
          while ( ! ( Enc28j60ReadReg ( BANKX_EIR ) & ( 1 << BANKX_EIR_TXIF ) )
              && ! ( Enc28j60ReadReg ( BANKX_EIR ) & ( 1 << BANKX_EIR_TXERIF ) ) )
